 ...
 * English quote instruction prefix.
 *
...
describe('buildQuoteHint', () => {
  ...
  it('emits hint with the quoted message_id for a chat-scope quote-reply', () => {
    ...
    expect(out).toBe('[The user quoted a message. Run `botmux quoted om_quoted` if its content is needed.]\n');
  });
  ...
  it('emits hint when thread-scope user quotes a non-root sibling', () => {
    ...
    expect(out).toBe('[The user quoted a message. Run `botmux quoted om_sibling` if its content is needed.]\n');
  });
