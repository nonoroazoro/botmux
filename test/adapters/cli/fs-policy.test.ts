...
describe('multi-user filesystem policy', () => {
  it('allows credentials only inside the dedicated principal home', () => {
    ...
    expect(accessForPath(policy.rules, '/var/lib/finder/users/alice/home/.ssh/id_ed25519').access)
      .toBe('readWrite');
    expect(accessForPath(policy.rules, '/var/lib/finder/users/bob/home/.ssh/id_ed25519').access)
      .toBe('none');
