 ...
 * a regular-group topic the bot otherwise would not know exists. The hint points the CLI
 ...
 * instruction. It performs no Lark API calls at all; the daemon gate already
...
describe('buildTopicThreadContext (hint mode, zero-fetch)', () => {
  ...
  it('is a HINT, not a transcript: carries no count and no message bodies/senders', () => {
    ...
    expect(out).not.toMatch(/\d+\s*(?:messages?|items?)/i);
