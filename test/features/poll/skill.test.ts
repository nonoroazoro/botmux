...
describe('poll skill', () => {
  it('creates directly without exposing implementation details', () => {
    ...
    expect(skill?.content).toContain('This is the only supported creation path');
    expect(skill?.content).toContain('recover the same target, title, options, and description');
    expect(skill?.content).toContain('preserve them exactly');
    expect(skill?.content).toContain('That is the complete response');
    expect(skill?.content).toContain('without announcing intent or progress');
    ...
    expect(skill?.content).toContain('Do not inspect other skills, source code, processes, sessions, or history');
