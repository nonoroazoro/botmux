...
export function buildArchitectGoal(specPath: string, specJsonPath: string): string {
  ...
  return [
    ...
    '- An inputs entry may select ONE named product instead of the whole manifest: { "from": "<nodeId>", "select": { "name": "<manifest file name>" } } (or { "path": ... }). Prefer a selector when the downstream goal needs one file; this keeps the worker context small.',
    '- Preserve risk_gate=true as a humanGate prompt on the corresponding node.',
    '- Estimate per-node timeoutSec from task size. Err on the generous side because manifest completion is detected early: quick single-file edits about 600, typical research or writing about 1800, and long research, refactor, or audit tasks 3600 to 7200. The hard ceiling is 14400. Omit it only for trivial nodes; the default is 1800.',
    ...
    '- Never put `botmux send`, `botmux reply`, or direct Feishu/Lark OpenAPI send/reply commands in a goal node. Goal nodes only write artifacts such as result.json/final_message.md; add a downstream feishu-send/feishu-reply host node for the notification.',
    ...
    '- When the spec means "rework until verification passes" (fix then test, implement then review, or generate then critique then revise), model it as one composite loop node. Never use a back-edge; the outer graph must remain acyclic.',
    ...
    '- `output.from` is what downstream consumes, usually the worker product rather than the verifier output.',
    '- Set maxIterations to the plausible number of useful rework rounds; 3 is typical and 20 is the ceiling. Do not use a loop when verification is the final product, such as a one-shot audit or report. Use a plain node chain.',
    ...
    '- When the spec means "decide once, then take one forward path," model it with conditional edges. Do not use a loop or back-edge; loops are for reworking the same output.',
    ...
    '- Nodes on an inactive branch are skipped at zero worker cost, and the skip cascades downstream. Cover every decision value. If every sink is skipped, the run fails with allSinksSkipped. Give a "nothing to do" outcome a cheap terminal node when needed.',
    '- `triggerRule` controls a join over incoming edges: "all_success" requires every edge, "one_success" accepts any active edge, and { "quorum": N } requires at least N. One-success and quorum nodes receive only active upstream products, so their goals must tolerate partial inputs.',
    ...
    '- `model`: use a cheaper or faster model for light research, summary, or verification nodes, and reserve the strong default for code or synthesis. State the cost reasoning in architect-notes.md.',
    '- Permission is NOT configurable per node: every workflow worker requires CLI bypass permissions; never emit `permissionMode`.',
    '- `systemPromptAppend`: short node-specific guardrails (at most 8000 bytes), for example, "Analyze only; do not modify files." Do not duplicate the goal text here.',
    '',
