...
export const CONFIG_FIELDS: readonly ConfigFieldSpec[] = [
  ...
  { key: 'skillInjection', configKey: 'skillInjection', kind: 'enum', effect: 'next-session', clearable: true, enumValues: ['global', 'prompt', 'off'], hint: 'Skill 提供方式：prompt 随会话提供（默认），global 安装到 CLI 全局目录，off 只提供命令帮助。切换全局模式后需重启服务；unset 恢复默认设置。' },
  { key: 'defaultWorkingDir', configKey: 'defaultWorkingDir', kind: 'dir', effect: 'next-session', clearable: true, hint: '新话题默认工作目录（跳过仓库选择卡片）' },
  { key: 'brandLabel', configKey: 'brandLabel', kind: 'string', effect: 'immediate', clearable: true, hint: '卡片页脚品牌文案；unset 显示机器人名称' },
  ...
  { key: 'codexAppCleanInput', configKey: 'codexAppCleanInput', kind: 'boolean', effect: 'immediate', clearable: false, hint: '实验性：Codex Desktop 用户气泡只保留真实输入，会话上下文走隐藏上下文；默认 off，从下一次 turn 派发生效，不改已有历史' },
