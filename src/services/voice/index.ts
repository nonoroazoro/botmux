 ...
 * ogg/opus file.
 *
 * The OpenAI-compatible HTTP engine is pluggable through its base URL and does
 * not pull a heavyweight dependency into the package.
 */
...
function hasUsableCreds(v: VoiceConfig | undefined): VoiceConfig | null {
  if (!v) return null;
  const engine = v.engine ?? (v.openai ? 'openai' : undefined);
...
function effectiveSpeaker(v: VoiceConfig): string {
  return v.speaker || DEFAULT_OPENAI_SPEAKER;
}
...
export async function synthesizeVoicePcmForMessage(larkAppId: string | undefined, text: string): Promise<Pcm> {
  ...
  return openaiSynthesizePcm(cfg.openai as OpenAITtsConfig, spoken, { speaker, rate: cfg.rate });
}
