...
export const messages: Record<string, string> = {
  ...
  'ai.routing.usage_history': '- When a request may depend on earlier messages, such as “look into this issue”, “continue”, or “do what we discussed”, proactively run `botmux history` for the current group or direct-message conversation before acting. Do not ask the user to repeat context. In a thread, use `botmux history --scope ambient` when the background may be outside the thread. If the context is not found and `hasMore=true`, read the next page with the returned `nextCursor`.',
  ...
  'ai.shell.helpers': 'Helpers: `botmux history` (read paginated history for the current group or direct message), `botmux quoted <message_id>` (fetch a quoted message only when the prompt header shows `[user quoted message ...]`), `botmux bots list` (list currently available collaborator bots). When a request may depend on earlier messages, such as “look into this issue”, “continue”, or “do what we discussed”, proactively run `botmux history` before acting instead of asking the user to repeat context. In a thread, use `--scope ambient` when the background may be outside the thread. If `hasMore=true`, continue with `--cursor <nextCursor>` when needed.',
  ...
  'card.capability.title.delete.bot': 'Confirm deleting this team {type}',
  ...
  'card.capability.button.personal.save': 'Save only for me',
  'card.capability.button.personal.update': 'Update only for me',
  ...
  'card.capability.button.delete.bot': 'Delete team content',
  ...
  'prompt.history_lookup.group_thread': '[History context: this session came from a group thread. When the request may depend on earlier messages, such as “look into this bug”, “continue”, or “do what we discussed”, you MUST read history before analyzing or acting. Use `botmux history` for the current thread. If the reference likely points to group discussion outside this thread, use `botmux history --scope ambient`. If the context is not found and `hasMore=true`, read the next page with the same scope and `--cursor <nextCursor>`. Do not ask the user to repeat context. Skip history when the request is self-contained.]',
  'prompt.history_lookup.group_chat': '[History context: this session came from a group conversation. When the request may depend on earlier group messages, such as “look into this bug”, “continue”, or “do what we discussed”, you MUST run `botmux history` before analyzing or acting. If the context is not found and `hasMore=true`, read the next page with `--cursor <nextCursor>`. Do not ask the user to repeat context. Skip history when the request is self-contained.]',
  'prompt.history_lookup.p2p': '[History context: this session came from a direct message. When the request may depend on earlier direct-message history, such as “look into this bug”, “continue”, or “do what we discussed”, you MUST run `botmux history --scope chat` before analyzing or acting. If the context is not found and `hasMore=true`, read the next page with the same scope and `--cursor <nextCursor>`. Do not ask the user to repeat context. Skip history when the request is self-contained.]',
