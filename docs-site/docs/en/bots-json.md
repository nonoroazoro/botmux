...
| `personalityReactions` | Model-selected reactions for the exact user message that started the current turn. Missing = enabled for bots with Lark transport; `false` = disabled. Supported reactions are `yes`, `no`, `heart`, `like`, and `done`; see [Bot Personality](/en/personality) |
